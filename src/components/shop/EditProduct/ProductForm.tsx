...
import { actionUpdateProduct } from "store/product/action";
...
export const ProductForm: React.FC = () => {
  ...
  const token = useAppSelector(
    ({ authentication }) => authentication.authUser?.data.accessToken
  );
  const fileListDefault = useAppSelector(({ product }) =>
    product.product?.productPictures?.map((file: any, index: any) => {
      if (index === 0) {
        return {
          uid: file?.includes("http")
            ? file
            : process.env.REACT_APP_BOOK_ECOMMERCE_HOST + file,
          name: "main-picture.png",
          status: "done",
          url: file?.includes("http")
            ? file
            : process.env.REACT_APP_BOOK_ECOMMERCE_HOST + file,
          thumbUrl: file?.includes("http")
            ? file
            : process.env.REACT_APP_BOOK_ECOMMERCE_HOST + file,
        };
      }
  ...
  const productFormValues = useMemo(() => {
    const date = product?.specs?.publicationDate
      ? product?.specs?.publicationDate.split("/")
      : "";
    return product
      ? {
          name: product.name,
      ...
      const product: any = {
        ...
        specs: [
          { k: "author", v: values.author },
          {
            k: "publicationDate",
            v: `${
              values.publicationDate.$D <= 9
                ? "0" + values.publicationDate.$D
                : values.publicationDate.$D
            }/${
              values.publicationDate.$M + 1 <= 9
                ? "0" + (values.publicationDate.$M + 1)
                : values.publicationDate.$M + 1
        ...
        productPictures: productImages?.map((image: any) =>
          image.replace("temp", "images")
        ),
      };
      dispatch(actionUpdateProduct(slug as string, product))
        .then((success: any) => {
          setLoading(false);
    ...
    <Card>
      ...
      >
        <Row gutter={{ xs: 10, md: 15, xl: 30 }}>
          ...
          <Col span={24}>
            <VariantForm initialValue={{ attributes: product?.variants }} />
          </Col>
        </Row>
      </BaseButtonsForm>
    </Card>
