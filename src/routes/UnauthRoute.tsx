...
const UnauthRoute: FC<Props> = () => {
  ...
  if (isSeller) return <Navigate to={SELLER_DASHBOARD_PATH  + SELLER_DASHBOARD_PRODUCTS_SUBPATH} />;
  
  return isAuth ? (
    <Navigate to={SELLER_DASHBOARD_PATH + SELLER_DASHBOARD_PRODUCTS_SUBPATH} />
  ) : (
    <Outlet />
  );
};
